import {
  ...
  emailDivider, emailButton, emailMeta,
} from "./base";
...
export function submissionReceivedEmail(d: SubmissionEmailData): string {
  ...
  return emailBase({
    ...
    body: `
      ...
      ${emailP(`A researcher has submitted a report to <strong style="color:#fafafa;">${d.programName}</strong>.`)}
      ${emailDivider()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${emailMeta("Title",      `<span style="color:#fafafa;">${d.submissionTitle}</span>`)}
        ${emailMeta("Severity",   emailBadge(d.severity.toUpperCase(), sevColor(d.severity)))}
        ${emailMeta("Researcher", d.researcherName)}
...
export function submissionAcceptedEmail(d: SubmissionEmailData): string {
  ...
  return emailBase({
    ...
    body: `
      ...
      ${emailP(`<strong style="color:#fafafa;">${d.orgName}</strong> has reviewed and accepted your vulnerability report.`)}
      ${emailDivider()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
      ...
      ${d.note ? `
        <div style="background:#0d2b29;border:1px solid #1a4340;border-radius:8px;padding:16px;margin-bottom:20px;">
          ...
          <p style="margin:0;font-size:14px;color:#a1a1aa;line-height:1.6;">${d.note}</p>
        </div>` : ""}
...
export function submissionRejectedEmail(d: SubmissionEmailData): string {
  ...
  return emailBase({
    ...
    body: `
      ...
      ${emailP(`After review, <strong style="color:#fafafa;">${d.orgName}</strong> has decided not to accept this report.`)}
      ${emailDivider()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${emailMeta("Report",  `<span style="color:#fafafa;">${d.submissionTitle}</span>`)}
        ${emailMeta("Program", d.programName)}
...
export function submissionNeedsInfoEmail(d: SubmissionEmailData): string {
  ...
  return emailBase({
    ...
    body: `
      ...
      ${emailP(`<strong style="color:#fafafa;">${d.orgName}</strong> is reviewing your report and needs clarification before proceeding.`)}
      ${emailDivider()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${emailMeta("Report",  `<span style="color:#fafafa;">${d.submissionTitle}</span>`)}
        ${emailMeta("Program", d.programName)}
...
export function submissionDuplicateEmail(d: SubmissionEmailData): string {
  ...
  return emailBase({
    ...
    body: `
      ...
      ${emailP(`<strong style="color:#fafafa;">${d.orgName}</strong> has determined this report is a duplicate of a previously submitted finding.`)}
      ${emailDivider()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${emailMeta("Your Report", `<span style="color:#fafafa;">${d.submissionTitle}</span>`)}
        ${emailMeta("Program",     d.programName)}
        ${emailMeta("Status",      emailBadge("DUPLICATE", "zinc"))}
        ${d.duplicateTitle ? emailMeta("Original", `<span style="color:#a1a1aa;">${d.duplicateTitle}</span>`) : ""}
      </table>
      ${d.note ? `
...
export function rewardApprovedEmail(d: RewardEmailData): string {
  ...
  return emailBase({
    ...
    body: `
      ...
      ${emailP(`<strong style="color:#fafafa;">${d.orgName}</strong> has approved your reward. Payment will be processed shortly.`)}
      ${emailDivider()}
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        ${emailMeta("Report",  `<span style="color:#fafafa;">${d.submissionTitle}</span>`)}
        ${emailMeta("Program", d.programName)}
