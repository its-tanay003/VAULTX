...
async function baseSubmissionsQuery(supabase: ReturnType<typeof createClient>, f: ReportFilters) {
  let q = supabase
    .from("submissions")
...
export async function metricBugsSubmitted(f: ReportFilters): Promise<{ label: string; value: number }[]> {
...
export async function metricBugsResolved(f: ReportFilters): Promise<{ label: string; value: number }[]> {
  const supabase = createClient();
  const { data } = await baseSubmissionsQuery(supabase, f);
  const resolved = (data ?? []).filter((s) => ["accepted", "resolved"].includes(s.status));
  const byDay = new Map<string, number>();
...
export async function metricSeverityDistribution(f: ReportFilters): Promise<{ label: string; value: number }[]> {
  const supabase = createClient();
  const { data } = await baseSubmissionsQuery(supabase, f);
...
export async function metricPayoutTotals(f: ReportFilters): Promise<{ label: string; value: number }[]> {
  const supabase = createClient();
...
// timing.
export async function metricAvgResponseTime(f: ReportFilters): Promise<{ label: string; value: number }> {
  const supabase = createClient();
  const { data } = await baseSubmissionsQuery(supabase, f);
  const triaged = (data ?? []).filter((s) => s.status !== "new");
  if (!triaged.length) return { label: "Avg Response Time (hrs, approx.)", value: 0 };

  const totalHours = triaged.reduce((sum, s) => {
...
export async function metricResearcherActivity(f: ReportFilters): Promise<{ label: string; value: number }[]> {
  const supabase = createClient();
  const { data } = await baseSubmissionsQuery(supabase, f);
  ...
  const nameMap = new Map((profiles ?? []).map((p) => [p.id, p.full_name ?? p.username ?? "Unknown"]));

  return Array.from(byResearcher.entries())
    .map(([id, value]) => ({ label: nameMap.get(id) ?? "Unknown", value }))
    .sort((a, b) => b.value - a.value)
    .slice(0, 20);
}

export async function metricResearcherLeaderboard(f: ReportFilters): Promise<
...
> {
  ...
  for (const r of rewards ?? []) {
    const entry = byResearcher.get(r.researcher_id) ?? { bugsFound: 0, totalPaid: 0, accepted: 0 };
    entry.totalPaid += r.amount;
    byResearcher.set(r.researcher_id, entry);
  }
...
export async function metricProgramRoi(f: ReportFilters): Promise<{ label: string; value: number }[]> {
  const supabase = createClient();
  ...
  for (const program of programs ?? []) {
    ...
    const validCount = (subs ?? []).filter((s) => ["accepted", "resolved"].includes(s.status)).length;
...
export async function metricSlaCompliance(f: ReportFilters): Promise<
