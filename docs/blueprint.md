...
# Master Blueprint & Design Document
...
* **Dynamic Decay Solver**: Rewards early solver speeds by dynamically decays points on subsequent solves.
...
### Prompt 2: Severity Classification & CVSS Estimator
```
System Prompt:
You are a security triager. Rate the severity of the provided vulnerability report based on CVSS v3.1 metrics.
Provide:
1. Severity category: Critical, High, Medium, Low, or Informational.
2. Estimated CVSS Score (0.0 to 10.0).
3. Metric breakdown (AV, AC, PR, UI, S, C, I, A).
4. A 1-sentence reasoning summary.
...
### Prompt 3: AI Red Team Target Explorer
```
System Prompt:
You are an autonomous AI Red Team agent probing a repository structure.
Analyze the target's description or file listing. Propose 3 potential high-risk attack vectors to focus scanning.
Respond with:
{
  "attackVectors": [
     {"name": string, "path": string, "reasoning": string}
  ]
}
...
### Prompt 4: PTaaS Test Plan Drafter
```
System Prompt:
Given the pentesting engagement details and goal, output a structured JSON list of 5 concrete verification steps.
{
  "testSteps": [
     {"step": number, "action": string, "expectedResult": string}
  ]
}
...
- **No Placeholders in Code**: Ensure all helper variables, mock IDs, and API triggers run real parsing. Do not commit inline mocks in production paths.
- **Fail-Open Rate Limiter**: Rate limiters must fail open on Upstash outages to ensure high availability of critical bug submissions.
- **Strict Role Boundaries**: Never permit researchers to fetch org triage stats. Verify that RLS queries enforce researcher restrictions globally.
- **Human Reward Gating Constraint**: Payouts must raise database exceptions if a human user ID is not recorded in the `approved_by` column.
- **Next.js Hydration Mismatches**: Standardize theme toggles to delay rendering until elements are mounted on the client-side.

---

## 9. IMPROVEMENTS & FUTURE UPGRADES

1. **GitHub OAuth Hook Extensions**: Map public repositories dynamically via user accounts rather than manual form links.
2. **Dynamic Decay Scores**: Decay solver rates dynamically as solve numbers scale on Jeopardy challenges.
3. **Advanced AI Remediation Suggestions**: Let triagers generate patch diffs automatically.
...
1. Add a Pages project linked to your GitHub repository.
2. Build Settings:
   - Framework preset: `Next.js` (compiled for Cloudflare Workers/Pages V8 isolate environment via Wrangler)
   - Build Command: `npm run build`
   - Output directory: `.open-next/assets`
3. Add environment variables in Cloudflare Console.
4. Set Compatibility Flag: `nodejs_compat` to enable standard crypto/stream libraries.

### Local Seeding Runbook
Run to setup mock accounts and structures:
```bash
npm run seed
```
...
The Settings module utilizes a layout with vertical sub-navigation. Available sections:
* **Profile**: Update full name, username, bio, website, and social handles (GitHub/Twitter).
* **Organization**: Manage organization name, billing details, and view current member roles.
* **Notifications**: Configure email and push notification preferences for new submissions, triage updates, and payment alerts.
* **Security**: Reset passwords, enable Multi-Factor Authentication (MFA), and view active login sessions.
* **API Keys**: Manage developer API tokens, assign scopes (`read:submissions`, `write:submissions`, etc.), and copy keys.
* **Integrations**: Connect external tools (GitHub App, Jira, Slack webhook).
* **Privacy**: Opt-in/out of AI assistant features (Consent Gating).
* **Danger Zone**: Delete account or organization data permanently with confirmation prompts.
